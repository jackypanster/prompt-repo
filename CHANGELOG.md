...
## [v0.8.0] - 2024-12-19 - 主页和提示词列表页面

### ✅ 新增功能
- **主页和提示词列表页面**: 实现完整的提示词浏览功能
  - 主页展示提示词列表，支持分页、筛选、排序
  - 分类筛选页面 `/category/{name}` 按分类浏览提示词
  - 标签筛选页面 `/tag/{name}` 按标签浏览提示词
  - 响应式设计，支持移动端和桌面端
...
- **用户友好的界面设计**:
  - 提示词卡片展示，包含标题、描述、内容预览
  - 精选提示词特殊标识
  - 分类和标签链接导航
  - 面包屑导航和空状态友好提示

- **服务器端渲染**: 使用Jinja2模板引擎实现SEO友好的HTML页面

### 🔧 技术改进
- **CRUD功能扩展**: 提示词查询函数支持排序参数
- **数据模型**: 添加按分类名称和标签名称查询的便捷函数
- **模板引擎**: 修复Jinja2模板中min函数使用问题

### 📝 验收完成
- ✅ 23个集成测试全部通过，覆盖：
  - 主页功能 (访问、分页、排序、筛选)
  - 分类页面功能 (访问、分页、排序、404处理)
  - 标签页面功能 (访问、分页、排序、404处理)
  - 页面内容显示 (提示词列表、精选标识、链接)
  - 响应格式验证 (HTML内容类型、UTF-8编码)
  - 错误处理 (无效参数、不存在资源)
